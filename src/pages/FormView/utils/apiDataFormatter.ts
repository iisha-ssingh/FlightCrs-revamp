...
export const flightDetailsFormatter = (
  ...
  const { flightTripId, ticketingSource, airline, flightNumber } =
    flightDetails;
  return {
    subTripId: {
...
export const populatePrefetch = (
  payload: Record<string, any>,
): typeof INITIAL_PREFETCH => {
  const result = { ...INITIAL_PREFETCH };

  for (const key in INITIAL_PREFETCH) {
    if (key in INITIAL_PREFETCH && key in payload) {
      result[key as keyof typeof INITIAL_PREFETCH] = payload[key].map(
        (item: any) => {
