...
const NetworkLayer = (() => {
    let isNetworkConnected: boolean | null = false;
    ...
    // TODO: Web compat implementation required
    ...
    const createHttpError = (status: number, response: ApiResponse): CustomError => {
    ...
    const handleFetchError = (error: unknown, extraInfo: {
        logErrorToRemote: boolean;
        ...
        if (error instanceof TypeError && error.message === STRINGS.NETWORK_REQUEST_FAILED) {
